...
export class NoteMapper {
  constructor() {
    ...
    this.pentatonicBase = [60, 62, 64, 67, 69]; // C4, D4, E4, G4, A4
  ...
  mapImpact(x, z, strength) {
    ...
    // X-axis -> Note selection (5 notes across the width)
    const noteIndex = Math.floor(normalizedX * 5);
    const clampedNoteIndex = Math.min(noteIndex, 4);
    ...
    // Calculate MIDI note
    const baseMidi = this.pentatonicBase[clampedNoteIndex];
    const midiNote = baseMidi + (clampedOctaveOffset - 1) * 12;
